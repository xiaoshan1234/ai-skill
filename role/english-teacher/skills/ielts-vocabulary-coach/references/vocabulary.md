...
- `ielts_vocabulary_personal_import`: import up to 500 entries from JSON or CSV. Content is imported; review progress is not.
- `ielts_vocabulary_personal_export`: export JSON or CSV content for a local Agent or user-owned backup. Review progress is excluded.
- `ielts_vocabulary_personal_update`: correct an entry or update its learning state.
- `ielts_vocabulary_personal_record_review`: record `again`, `hard`, or `good` after the learner actually answers a card.
- `ielts_vocabulary_personal_delete`: remove one entry after confirmation.

## Built-in Wordbook Tools

- `ielts_vocabulary_builtin_progress`: read progress for `core`, `listening`, or `reading`.
- `ielts_vocabulary_builtin_prepare_cards`: prepare cards from one built-in wordbook.
...
For local flashcard practice, choose exactly one source and call `ielts_vocabulary_personal_prepare_cards` or `ielts_vocabulary_builtin_prepare_cards`. Return the card data in the conversation. Do not provide a browser link unless the user explicitly asks to open the web app.

For an interactive local session, use `../workflows/vocabulary-session/WORKFLOW.md`: one card at a time, active recall before reveal, concise feedback, and the matching personal or built-in review tool after each answered card.
