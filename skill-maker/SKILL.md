...
description: >-
  ...
  Use when creating or refactoring skills for OpenCode, Cursor, Claude Code, or
  compatible agents; when the user asks how to write skills, SKILL.md format, or
  skill naming and permissions.
...
| **存放位置** | 项目：`.opencode/skills/<name>/` 或 `.cursor/skills/<name>/`；全局：`~/.config/opencode/skills/` 等。 |
...
可选：`license`、`compatibility`（如 `opencode`）、`metadata`（字符串键值）。
...
6. **延伸阅读（可选）**：大段参考放到同目录 `reference.md` 等，**SKILL.md 只一层链接**，避免深嵌套。
...
- 主文件 **SKILL.md** 保持可扫读；长手册拆到 **`reference.md` / `examples.md`**，需要时再读。
...
在 `opencode.json` 用 `permission.skill` 控制 `allow` / `ask` / `deny`；不需要 Skill 的 Agent 可设 `tools.skill: false`。与组织策略冲突时以项目配置为准。
