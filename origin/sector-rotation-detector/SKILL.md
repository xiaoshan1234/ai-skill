...
本技能回答的是**宏观驱动的长期行业配置**（6–12个月），不是某一天的板块涨跌原因。

- **当用户问“为什么XX板块今天大涨/大跌”** → 先查实时行情 + 财经新闻（财联社、东方财富、同花顺），再找政策/事件催化。这类问题属于**短期异动解读**，应使用 `cn-stock` MCP 或 `findata-toolkit-cn` 获取实时数据，并直接浏览主流财经网站找最新新闻，而不是套用本技能的6步周期分析。
- **当用户问“未来6–12个月该超配/低配哪些行业”“现在处于经济周期哪个阶段”** → 使用本技能的完整流程。

如果一次提问中两者交织，先用实时工具给出当日原因，再补充周期观点。
