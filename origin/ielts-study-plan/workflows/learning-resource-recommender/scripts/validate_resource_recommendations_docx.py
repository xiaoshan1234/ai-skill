#!/usr/bin/env python3
"""Validate an IELTS resource recommendation DOCX."""
...
def main() -> None:
    ...
    if tables < 1:
        raise SystemExit("Expected at least one recommendation table")
    if args.plan_json:
        plan = json.loads(args.plan_json.read_text(encoding="utf-8"))
        recommendations = [item for item in plan.get("recommendations", []) if isinstance(item, dict)]
        if not recommendations:
            raise SystemExit("Plan must contain recommendations")
        if len(recommendations) > 7:
            raise SystemExit("Too many recommendations; keep the list focused")
        for item in recommendations:
            for key in ("title", "url", "how_to_use"):
