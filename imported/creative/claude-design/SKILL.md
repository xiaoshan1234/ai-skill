...
## Design Workflow: claude-design as the Umbrella

This skill (`claude-design`) is the class-level umbrella for all design work.
The narrow skills `popular-web-designs` and `design-md` have been absorbed into it.

| What you want | How to get it |
|---|---|
| Design process + taste + one-off artifact | Use **claude-design** (this skill) |
| Match a known brand's look (54 systems: Stripe, Linear, Vercel, etc.) | Use **claude-design** + load templates from `templates/` (absorbed from `popular-web-designs`) |
| Author a formal DESIGN.md token spec file | Use **claude-design** + the `design-md-starter.md` template absorbed from `design-md` |

Design system templates are now at `templates/` (e.g. `templates/stripe.md`, `templates/linear.app.md`).
The original narrow skills are preserved at `~/.hermes/skills/.archive/popular-web-designs/` and `~/.hermes/skills/.archive/design-md/`.
